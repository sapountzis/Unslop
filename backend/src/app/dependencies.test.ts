  ...
  const base: AppDependencies = {
    ...
    authMiddleware: async (c, next) => {
      ...
      auth: {
        ...
        getCurrentUser: mock(async () => ({ id: 'user-1', email: 'user@example.com', plan: 'free', planStatus: 'inactive' })),
      },
      feedback: {
      ...
      polar: {
        ...
        claimWebhookDelivery: mock(async () => ({ webhookId: 'wh', isDuplicate: false })),
      ...
      quota: {
        ...
        tryConsumeQuota: mock(async () => ({ allowed: true, remaining: 299, periodStart: '2026-02-01' })),
...
describe('app dependency wiring', () => {
  ...
  it('routes call injected service functions', async () => {
    const classifySingle = mock(async () => ({ post_id: 'svc-1', decision: 'keep' as const, source: 'llm' as const }));
    const app = createApp(
      makeDeps({
        services: {
          classification: {
