...
describe("runtime config", () => {
	it("enforces required env vars", () => {
		...
		expect(() =>
			loadRuntimeConfig(env, { allowMissingSecrets: false }),
		).toThrow("Missing required environment variable: APP_URL");
	});
	...
	it("parses numeric values with defaults", () => {
		const config = loadRuntimeConfig(
			baseEnv({ PORT: undefined, FREE_MONTHLY_LLM_CALLS: undefined }),
		);
