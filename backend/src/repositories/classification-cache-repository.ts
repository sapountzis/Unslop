...
export interface ClassificationCacheRepository {
  ...
  upsertSuccess: (input: UpsertClassificationCacheSuccessInput) => Promise<void>;
}
...
export function createClassificationCacheRepository(
  ...
  async function upsertSuccess(input: UpsertClassificationCacheSuccessInput): Promise<void> {
    const now = new Date();
