...
export async function findFreshPostDecision(
  postId: string,
  cacheExpiry: Date
...
export async function findFreshPostDecisions(
  postIds: string[],
  cacheExpiry: Date
...
export async function upsertPostClassification(input: UpsertPostClassificationInput): Promise<void> {
  await db
    .insert(posts)
    .values({
      postId: input.postId,
