...
export function createStatsRepository(
	...
	async function countByDecisionForRange(
		...
		const whereClause = createdAtGte
			? and(
					...
					gte(userActivity.createdAt, createdAtGte),
				)
			: eq(userActivity.userId, userId);
	...
	async function getDailyBreakdownForUser(
		userId: string,
		lookbackStart: Date,
	): Promise<DailyBreakdownRow[]> {
		return db
			...
			.where(
				and(
					eq(userActivity.userId, userId),
					gte(userActivity.createdAt, lookbackStart),
				),
			)
		...
		const rows = await db
			...
			.where(
				and(
					...
					eq(userUsage.monthStart, periodStart),
				),
			)
			.limit(1);
