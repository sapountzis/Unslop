	...
	if (url.includes("/v1/products/")) {
		return Promise.resolve(
			new Response(
				JSON.stringify({
					prices: [
						{ id: "price_123", type: "recurring", recurring_interval: "month" },
					],
				}),
			),
		);
...
describe("createCheckoutSession", () => {
	...
	it("throws ALREADY_PRO for active pro user", async () => {
		...
		const service = buildService();
		await expect(service.createCheckoutSession("u1")).rejects.toThrow(
			"ALREADY_PRO",
		);
	});
...
describe("webhook handlers", () => {
	...
	it("handleSubscriptionCanceled sets PRO/CANCELED", async () => {
		...
		await service.handleSubscriptionCanceled(
			mockPayload({ status: "canceled" }).data,
		);
	...
	it("handleSubscriptionPastDue sets PRO/PAST_DUE", async () => {
		...
		const service = buildService();
		await service.handleSubscriptionPastDue(
			mockPayload({ status: "past_due" }).data,
		);
	...
	it("handleSubscriptionUpdated routes correctly", async () => {
		...
		await service.handleSubscriptionUpdated(
			mockPayload({ status: "active" }).data,
		);
