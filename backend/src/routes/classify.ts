...
function validateNodeGraph(nodes: MultimodalPostNode[]): NodeValidationIssue[] {
  ...
  const computeDepth = (nodeId: string, visiting: Set<string>): number | null => {
...
const classifyNodeSchema = z.object({
  id: z.string().min(1),
...
const classifyImageAttachmentSchema = z.object({
  node_id: z.string().min(1),
  kind: z.literal('image'),
  sha256: z.string().regex(SHA256_HEX_PATTERN),
  mime_type: z.string().min(1),
  base64: z.string().regex(BASE64_PATTERN).superRefine((value, ctx) => {
...
const classifyPdfAttachmentSchema = z.object({
  node_id: z.string().min(1),
  kind: z.literal('pdf'),
...
const classifyNodesSchema = z.array(classifyNodeSchema)
  .min(1)
...
export const classifyPostSchema = z.object({
  ...
  attachments: z.array(classifyAttachmentSchema)
...
export function createClassifyRoutes(deps: ClassifyRoutesDeps): Hono {
  ...
  classify.post('/v1/classify', deps.authMiddleware, zValidator('json', classifySchema), async (c) => {
    ...
    try {
      const result = await deps.classificationService.classifySingle(user.sub, payload.post);
  ...
  classify.post('/v1/classify/batch', deps.authMiddleware, zValidator('json', batchClassifySchema), async (c) => {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          outcomes = await deps.classificationService.classifyBatch(user.sub, payload.posts);
        } catch (_error) {
