...
function createOpenAiCapture() {
  ...
  const openai = {
    chat: {
      completions: {
        parse: async (params: { model: string; messages: Array<{ role: string; content: unknown }> }) => {
          model = params.model;
...
function makeTextPost(): PostInput {
  return {
    ...
    nodes: [
      ...
      { id: 'repost-1', parent_id: 'root', kind: 'repost', text: 'Repost context.' },
    ],
    attachments: [],
  };
}
...
function makeMultimodalPost(): PostInput {
  return {
    ...
    nodes: [{ id: 'root', parent_id: null, kind: 'root', text: 'Main post body.' }],
    attachments: [
      {
        node_id: 'root',
        kind: 'image',
...
function makePdfOnlyPost(): PostInput {
  return {
    ...
    nodes: [{ id: 'root', parent_id: null, kind: 'root', text: 'PDF backed text context.' }],
    attachments: [
      {
        node_id: 'root',
        kind: 'pdf',
...
async function skipIfNoRealLlmKey(): Promise<boolean> {
  if (process.env.RUN_REAL_LLM_INTEGRATION !== 'true') {
    console.log('Skipping llm integration test: set RUN_REAL_LLM_INTEGRATION=true to enable.');
    return true;
  }
...
describe('LLM Service (integration)', () => {
  ...
  it('callModel uses vlm model and includes image content parts when images exist', async () => {
    const capture = createOpenAiCapture();
    await callModel(capture.openai, testConfig, makeMultimodalPost(), testLogger);
    ...
    const imagePart = userContent.find((part: unknown) => (
      typeof part === 'object'
      && part !== null
  ...
  it('selectModelForPayload routes by attachment presence', () => {
    ...
    expect(selectModelForPayload(withAttachment, testConfig)).toBe('vlm-model-1');
