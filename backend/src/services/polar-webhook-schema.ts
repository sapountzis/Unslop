...
export function normalizeSubscriptionData(
	...
	const userId =
		readString(parsed.data.metadata?.user_id) ??
		readString(parsed.data.metadata?.userId);
	if (!userId) {
		return null;
	}
	...
	return {
		...
		periodStart: parseDate(
			parsed.data.current_period_start ?? parsed.data.currentPeriodStart,
		),
		periodEnd: parseDate(
			parsed.data.current_period_end ?? parsed.data.currentPeriodEnd,
		),
	};
}

export function mapSubscriptionStatusToAction(
