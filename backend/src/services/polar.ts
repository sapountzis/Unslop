...
export interface PolarService {
  ...
  claimWebhookDelivery: (payload: BillingWebhookPayload) => Promise<WebhookDeliveryClaim>;
...
export function createPolarService(deps: PolarServiceDeps): PolarService {
  ...
  async function getPriceId(): Promise<string> {
    ...
    if (cachedPriceId && nowMs - cachedPriceIdTimestamp < POLAR_PRICE_CACHE_TTL_MS) {
      return cachedPriceId;
    }

    const response = await fetchImpl(`${deps.config.apiBase}/v1/products/${deps.config.productId}`, {
    ...
    const price = product.prices.find((entry) => entry.type === 'recurring' && entry.recurring_interval === 'month');
    if (!price) {
  ...
  async function createCheckoutSession(userId: string): Promise<{ checkout_url: string }> {
  ...
  function buildWebhookDeliveryKey(payload: BillingWebhookPayload): string {
    const subscriptionId = getSubscriptionIdFromWebhookData(payload.data) ?? 'unknown';
    return `${payload.type}:${subscriptionId}:${asIsoTimestamp(payload.timestamp)}`;
  }

  async function claimWebhookDeliveryById(input: ClaimWebhookDeliveryByIdInput): Promise<WebhookDeliveryClaim> {
  ...
  async function releaseWebhookDeliveryById(webhookId: string): Promise<void> {
    await deps.db.delete(webhookDeliveries).where(eq(webhookDeliveries.webhookId, webhookId));
  }

  async function claimWebhookDelivery(payload: BillingWebhookPayload): Promise<WebhookDeliveryClaim> {
  ...
  async function setUserTier(
    ...
    await deps.db.update(users).set({ plan, planStatus, ...extra }).where(eq(users.id, userId));
  }
  ...
  async function handleSubscriptionCanceled(data: unknown): Promise<void> {
    const sub = normalizeSubscriptionData(data);
    if (!sub) {
      serviceLogger.warn('billing_webhook_invalid_subscription_canceled_payload');
      return;
    }
    ...
    serviceLogger.info('subscription_canceled', { userId: sub.userId, accessUntil: sub.periodEnd });
  }

  async function handleSubscriptionUncanceled(data: unknown): Promise<void> {
    const sub = normalizeSubscriptionData(data);
    if (!sub) {
      serviceLogger.warn('billing_webhook_invalid_subscription_uncanceled_payload');
      return;
    }
  ...
  async function handleSubscriptionRevoked(data: unknown): Promise<void> {
    const sub = normalizeSubscriptionData(data);
    if (!sub) {
      serviceLogger.warn('billing_webhook_invalid_subscription_revoked_payload');
      return;
    }
  ...
  async function handleSubscriptionPastDue(data: unknown): Promise<void> {
    const sub = normalizeSubscriptionData(data);
    if (!sub) {
      serviceLogger.warn('billing_webhook_invalid_subscription_past_due_payload');
      return;
    }
  ...
  async function handleSubscriptionUpdated(data: unknown): Promise<void> {
    const sub = normalizeSubscriptionData(data);
    if (!sub) {
      serviceLogger.warn('billing_webhook_invalid_subscription_updated_payload');
      return;
    }
