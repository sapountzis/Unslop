...
export function createStatsService(deps: StatsServiceDeps): StatsService {
  ...
  async function getStats(userId: string) {
    ...
    const lookbackStart = new Date(today.getTime() - STATS_LOOKBACK_DAYS * 24 * 60 * 60 * 1000);
  ...
  async function getUsage(userId: string) {
    ...
    const currentUsage = await deps.statsRepository.getUsageForPeriod(userId, context.periodStart);
