...
export async function getUserInfoWithUsage(jwt: string): Promise<UserInfoWithUsage | null> {
...
export async function createCheckout(jwt: string): Promise<string | null> {
  return dedupedRequest('createCheckout', async () => {
    ...
    while (retries <= maxRetries) {
      try {
        const response = await fetchWithTimeout(`${API_BASE}/billing/create-checkout`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${jwt}`,
