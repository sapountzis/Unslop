...
function resolveBudgets(
	...
	return {
		...
		minImageDimension: dependencies.minImageDimension ?? MIN_IMAGE_DIMENSION,
...
async function resolveAttachment(
	...
	const ordinal =
		typeof record.ordinal === "number" && record.ordinal >= 0
			? record.ordinal
			: undefined;
	if (kind === "image") {
		return resolveImageAttachment(record, ordinal, dependencies, budgets);
	}
	if (kind === "pdf") {
		return resolvePdfAttachment(record, ordinal, dependencies, budgets);
	}
...
async function resolveImageAttachment(
	...
	if (isDefinitelyNotImage(originalMimeType)) {
		throw new Error(
			`image_fetch_failed:invalid_content_type:${originalMimeType}`,
		);
	}
...
function isDefinitelyNotImage(mimeType: string): boolean {
	return (
...
function toBase64(bytes: Uint8Array): string {
	...
	for (let i = 0; i < bytes.length; i += chunkSize) {
		...
		chunks.push(Array.from(chunk, (c) => String.fromCharCode(c)).join(""));
	}
