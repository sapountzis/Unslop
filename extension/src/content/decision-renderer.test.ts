...
class MockElement {
	...
	querySelector(selector: string): MockElement | null {
		if (
			selector === ":scope > .unslop-decision-label" ||
			selector === ".unslop-decision-label"
		) {
			return (
				this.children.find((child) =>
					child.className.includes("unslop-decision-label"),
				) ?? null
			);
		}
		return null;
...
describe("renderDecision", () => {
	...
	it("supports label mode for hide decision in local testing", () => {
		...
		renderDecision(post as MockHTMLElement, "hide", "post-2", {
			hideMode: "label",
		});
		...
		expect(post.getAttribute(ATTRIBUTES.decision)).toBe("hide");
		expect(
			post.querySelector(":scope > .unslop-decision-label"),
		).not.toBeNull();
	});
	...
	it("supports label mode for keep decision in local testing", () => {
		...
		renderDecision(post as MockHTMLElement, "keep", "post-3", {
			hideMode: "label",
		});
