...
const OPEN_LINKEDIN_FEED_ACTION =
	"Open https://www.linkedin.com/feed/ and rerun diagnostics.";
...
type LinkedInDiagnosticsDependencies = {
...
export class LinkedInDiagnosticsService implements PlatformDiagnosticsProvider {
	...
	constructor(dependencies: LinkedInDiagnosticsDependencies = {}) {
	...
	collectSnapshot(url: string): PlatformDiagnosticsSnapshot {
		...
		const checks: DiagnosticCheck[] = [
			createPlatformCheck({
				...
				nextAction: routeEligible ? "None." : OPEN_LINKEDIN_FEED_ACTION,
			}),
			createPlatformCheck({
...
export const linkedinDiagnostics: PlatformDiagnosticsProvider =
	new LinkedInDiagnosticsService();
