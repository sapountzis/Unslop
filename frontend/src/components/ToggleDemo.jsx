  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="mb-10 flex items-center justify-center gap-6">
        <span className={`font-mono text-xs uppercase tracking-widest transition-colors ${enabled ? 'text-gray-300' : 'text-gray-900'}`}>
          Noise
        </span>
        ...
        <span className={`font-mono text-xs uppercase tracking-widest transition-colors ${enabled ? 'text-[#4A6C48] font-bold' : 'text-gray-400'}`}>
          Signal
        </span>
      </div>
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    {/* Avatar Circle */}
                    <div className={`h-8 w-8 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] ${
                      card.type === 'signal' 
                        ? 'bg-[#4A6C48]/20 text-[#4A6C48]' 
                        : 'bg-[#C15C5C]/10 text-[#C15C5C]'
                    }`}>
                        {/* Abstract Avatar Icon */}
                        <div className={`w-2 h-2 rounded-full ${card.type === 'signal' ? 'bg-[#4A6C48]' : 'bg-[#C15C5C]'}`} />
                    </div>
                  ...
                  <p className={`font-serif text-lg leading-relaxed ${
                    card.type === 'signal' ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/80 italic'
                  }`}>
                    {card.text}
                  </p>
